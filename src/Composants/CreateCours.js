...
class CreateCours extends Component {
        ...
        componentDidMount(){
            if(this.state.id==='_add'){
                ...
                CourService.getCoursById(this.state.id).then((response)=>{
                  ...
                  this.setState({libelle: cours.libelle,
  ...
  <Form>
    ...
    <Form.Group widths={2}>
      ...
      <Form.Input label='Enseignant' placeholder='Enseignant' value={this.state.enseignantid} onChange={this.changeEnseignant}/>
      <Form.Input label='Type cours' placeholder='Type cours' value={this.state.typecourid} onChange={this.changeTypeCours}/>
    </Form.Group>
